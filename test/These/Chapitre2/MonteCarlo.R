...
for(ij in 1:nrow(model_extern)){
  ...
  Y<-foreach(i=1:n, .combine=rbind, .export=c("MDSVsim", "MDSVfit"), .packages = c("MDSV"), .options.snow = opts) %dopar% {
       ...
       X<-as.matrix(MDSVsim(N = N, K = N, para = para, ModelType = ModelType, LEVIER = LEVIER, n.sim = n.sim, 
                  n.start = n.start, m.sim = m.sim, rseed = NA)$sim.1$r_t)
       
       opt<-MDSVfit(N = N, K = N, data = X, para = para, ModelType = ModelType, LEVIER = LEVIER, ctrl=ctrl)
...
write.table(X[,index], paste0("MONTECARLO_K_",K,"_N_",N,".csv"), sep = ";")
...
for(ij in 1:nrow(model_extern)){
  ...
  Y<-foreach(i=1:n, .combine=rbind, .export=c("MDSVsim", "MDSVfit"), .packages = c("MDSV"), .options.snow = opts) %dopar% {
    ...
    opt<-MDSVfit(N = N, K = K, data = X, para = para, ModelType = ModelType, LEVIER = LEVIER, ctrl=ctrl)
...
write.csv(Y, paste0("MonteCarlo2_",ordi,"_K_",2,"_N_",5,".csv"), row.names=FALSE)

##### #Traitement des bases de donnees : PANEL C ET PANEL D


files.all<-Sys.glob("MonteCarlo2_*.csv")
...
ordi<-1
model<-expand.grid(ech=1,LEVIER=FALSE,K=c(1:K_max),N=N,N_T=T1,Np=0,loglik=0,AIC=0,BIC=0,omega=0,a=0,b=0,sigma=0,v0=0,l=0,theta_l=0)
