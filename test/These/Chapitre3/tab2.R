...
pb <- txtProgressBar(min=41, max = nrow(model), style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress = progress)

Y<- foreach(i=41:nrow(model), .export=c("MDSVfit","MDSVfilter"),.packages=c("MDSV"), .combine = "rbind",.options.snow=opts) %dopar% {
  start_time <- Sys.time()
...
write.csv(Y, paste0("All_estimations_suite_2.csv"), row.names=FALSE)
